...
def get_redirect_uri(request: Request) -> str:
    ...
    if IS_REPLIT:
        ...
        if replit_host:
            ...
            print(
                f"[Auth] REPLIT_DOMAINS={REPLIT_DOMAINS}, REPLIT_DEV_DOMAIN={REPLIT_DEV_DOMAIN}"
            )
...
async def login(request: Request, db: Session = Depends(get_db)):
    ...
    print(
        f"[Auth] Client ID: {oauth_config['client_id'][:20]}..."
        if oauth_config["client_id"]
        else "[Auth] Client ID: NOT SET"
    )
...
async def callback(
    ...
    print(
        f"[Auth Callback] Received callback - code: {bool(code)}, state: {bool(state)}, error: {error}"
    )
    ...
    if not code or not state:
        print(
            f"[Auth Callback] Missing params - code: {bool(code)}, state: {bool(state)}"
        )
    ...
    state_record = (
        ...
        .filter(
            OAuthStateModel.state == state, OAuthStateModel.expires_at > datetime.now()
        )
        .first()
    )
    ...
    print(
        f"[Auth Callback] Exchanging code for tokens at: {oauth_config['token_endpoint']}"
    )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(
                f"[Auth Callback] Token response status: {token_response.status_code}"
            )
    ...
    print(
        f"[Auth Callback] Extracted user data - id: {oauth_user_id}, email: {email}, name: {name}"
    )
    ...
    if provider == "google":
        user = (
            db.query(UserModel)
            .filter(UserModel.google_user_id == oauth_user_id)
            .first()
        )
    else:
        user = (
            db.query(UserModel)
            .filter(UserModel.replit_user_id == oauth_user_id)
            .first()
        )
    ...
    oauth = (
        db.query(OAuthModel)
        .filter(OAuthModel.user_id == user.id, OAuthModel.provider == provider)
        .first()
    )
